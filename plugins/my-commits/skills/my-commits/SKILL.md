...
## 1. Pick the period from the user's phrasing

| Phrasing | `--period` |
|---|---|
| "today", "commits today", or anything unclear | `today` |
| "yesterday" | `yesterday` |
| "this week", "this week's commits" | `week` (since Monday) |
| "this month" | `month` (since the 1st) |
...
- today / yesterday → one row per commit (`hash subject` | `+added −deleted`),
  oldest first, with a **Total** row last.
- this week / this month → one row per day (`Day | Commits | Lines`), oldest
